...
define-command -docstring "enable modeline" modeline-enable %{
  ...
  set-option global modelinefmt '{StatusLineValue}%opt{modeline_mode}{StatusLineInfo}%opt{modeline_modified}{StatusLineInfo}%opt{modeline_readonly}%val{bufname}  %opt{modeline_selections}  %val{cursor_line}:%val{cursor_char_column} 󰴼 %val{client}‧%val{session}'
